...
class Template:
    ...
    def process_code(self, source_code):
        ...
        try:
            ...
            #ls = {}
            #exec(sequential_code,globals(),ls)
            #print(ls)
            gui_module, hal_module,destination_module = self.generate_modules()
            builtin_modules = {"GUI": gui_module, "HAL": hal_module,"destination":destination_module, "time": time,'console': self.console, 'print': print_function}
            globl = globals()
            global_functions = globl.copy()
            global_functions.update(builtin_modules)
            exec(sequential_code,global_functions ,reference_environment)
            
            arr  = reference_environment["generatepath"]
            arrq = arr()
            print(arrq)
    ...
    def generate_modules(self):
        ...
        destination_module.destination.pose = self.data

        # Define GUI module
        gui_module = imp.new_module("GUI")
        gui_module.GUI = imp.new_module("GUI")
 
        gui_module.GUI.robotPose = self.gui.update_gui
        ...
        return gui_module, hal_module ,destination_module
